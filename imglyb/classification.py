...
StackAccessMode = autoclass( 'net.imglib2.view.StackView$StackAccessMode' )
PaintLabelsAndTrain = autoclass('net.imglib2.atlas.PaintLabelsAndTrain')
FunctorClassifier = autoclass('net.imglib2.python.atlas.FunctorClassifier')
...
class VigraClassification( PythonJavaClass ):
	__javainterfaces__ = [ 'net.imglib2.python.atlas.FunctorClassifier$ClassificationImplementation' ]

	def __init__( self, classifier, train_kwargs = {}, predict_kwargs = {} ):
		super( VigraClassification, self ).__init__()
	...
	@java_method( '(Lnet/imglib2/img/array/ArrayImg;Lnet/imglib2/img/array/ArrayImg;)V' )
...
if __name__ == "__main__":
	...
	print( 'img', img, img_np.dtype, util.Helpers.className( img.randomAccess().get() ) )
	
	
	cellDimensions = ( 64, 64, 2 )
	dimensions = img_np.shape[::-1]
	...
	vigra_proc_functor3 = VigraProcessingFunctor(
		source            = util.Views.extendBorder( img ),
		margin            = (3, 3, 3),
		store_constructor = DirtyVolatileOwningFloatUnsafe,
		functor           = vigra.filters.gaussianSmoothing,
		**{ 'sigma' : 1.0 }
		)
	proc3 = LambdaProcessor( vigra_proc_functor3 )
	processing_loader3 = ProcessingLoader( proc3, grid )

	proc_cache3 = create_caches( ttype.copy(), vtype.copy(), DiskCellCache, "processed-", grid, processing_loader3, PrimitiveType.FLOAT, AccessFlags.VOLATILE )
	proc_img3, proc_v_img3 = create_img_and_volatile_image( ttype.copy(), vtype.copy(), grid, proc_cache3, queue )
	feats = util.Views.concatenate( 3, StackAccessMode.DEFAULT, *[ cast( 'net.imglib2.RandomAccessibleInterval', util.Views.addDimension( rai, 0, 0 ) ) for rai in ( proc_img, proc_img2, proc_img3 ) ] )
	vfeats = util.Views.concatenate( 3, StackAccessMode.DEFAULT, *[ cast( 'net.imglib2.RandomAccessibleInterval', util.Views.addDimension( rai, 0, 0 ) ) for rai in ( proc_v_img, proc_v_img2, proc_v_img3 ) ] )
	

	# bdv = util.BdvFunctions.show( img, "Cached" )
	# print( "Where is bdv", bdv )
	# bdv.getBdvHandle().getViewerPanel().setDisplayMode( DisplayMode.SINGLE )
	# print( 'type', cast( util.Helpers.className( proc_v_img.randomAccess().get() ), proc_v_img.randomAccess().get() ).getRealFloat() )
	...
	# util.BdvFunctions.show( proc_v_img3, "processed3", util.BdvOptions.options().addTo( bdv ) )
